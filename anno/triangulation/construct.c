     ...
     int segnum;
{
  ...
  tr[t4].hi.y = (double) (T_INFINITY);
  tr[t4].hi.x = (double) (T_INFINITY);
  tr[t3].lo.y = (double) -1* (T_INFINITY);
  tr[t3].lo.x = (double) -1* (T_INFINITY);
	...
	{			/* Only one trapezoid below */
	  ...
	  if (FP_EQUAL(tr[t].lo.y, tr[tlast].lo.y) && 
	    ...
	    {		/* bottom forms a triangle */
          ...
          /*
          ap: replaced with the condition below
          because tmpseg was used without initialization ->
